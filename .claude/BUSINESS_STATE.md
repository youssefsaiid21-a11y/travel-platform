...
- 2026-07-16: Closed out BUG-0005, BUG-0007, and the two "pending" prod-DB
  ...
  before implementation (flagged that skipping Sentry telemetry on the new
  refusal branch would leave it unmeasurable and fail-open-silent if
  `expires_at` were ever unparseable - founder approved adding it), then an
  independent post-implementation review (PASS) and a clean
  `booking-safety-reviewer` pass. BUG-0007 got the telemetry instrumentation
  its 2026-07-14 investigation recommended (phase-tagged Sentry capture in
  `api/chat/route.ts`'s catch block) with no code change to `nl-parser.ts`
  itself, per that investigation's own warning against masking an
  unconfirmed infra gap as a fake fix. Both PRs (#12, #13) merged only
  after the founder named each PR number explicitly - a generic "yes to
  both, go ahead" was not sufficient (see calibration log below). Full gate
  re-run directly on the merged `main` (not trusted from either PR's own
  report): lint/tsc clean, 465/465 tests passing on a clean run (one run
  briefly showed 2 failures in an unrelated MFA/TOTP backup-codes test,
  not touched by either diff - didn't reproduce on immediate re-run,
  logged as a possible pre-existing flake, not a regression, not
  investigated further this session).

  The migrations turned out to need no action at all: `CLAUDE.md` said
  both `20260707215113_user_token_version` and
  `20260707224602_booking_json_columns` were "written but NOT yet applied
  to prod, needs explicit human approval." A read-only `prisma migrate
  status` plus a raw `_prisma_migrations` query (both required a specific,
  one-time founder go-ahead this session - see calibration log) showed all
  12 migrations, including these two, already applied with `finished_at`
  set and no rollback. `CLAUDE.md` was simply never updated when they
  actually ran - fixed directly (doc-only commit to `main`, no new ask
  needed under the existing rule for that class of change).
